...
Searchable IP Dropdown History: Remembers previously typed project tracking entries and provides a sleek, filtered drop-down combobox overlay that handles scrollbars perfectly.
...
Open Chrome Extensions Dashboard:

Open Google Chrome and go to chrome://extensions/.

Enable Developer Tools Framework:
...
Open a fresh, empty tracking ledger by visiting Google Sheets (https://sheets.new).

Set up headers across row 1 in Columns A through I exactly as shown below:
Date | Username | Name | Followers | Category | Tier | Type | IP | URL

In the top-level menu bar, select Extensions > Apps Script.

Erase any placeholder framework code block and drop in the complete scripting backend module:
...
🎯 Usage Manual
Navigate to any target creator profile webpage on Instagram Web version (https://instagram.com).
