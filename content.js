chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "scrape_profile") {
    let data = { username: "", name: "--", followers: "--", category: "--", url: window.location.href };
    try {
      const pathParts = window.location.pathname.split('/').filter(p => p);
      if (pathParts.length > 0) data.username = pathParts[0];
      ...
      if (header) {
        ...
        if (firstStatIndex > 0) {
          ...
          if (nameCandidate.toLowerCase() === 'verified') nameCandidate = lines[firstStatIndex - 2];
        ...
        if (followersIndex !== -1) {
          ...
          while (categoryIndex < lines.length) {
            ...
            if (lowerLine.includes('following') || lowerLine.includes('followed by') || lowerLine.includes('mutual')) categoryIndex++;
            else break;
          }
          if (categoryIndex < lines.length) {
            ...
            if (nextLine.length <= 40 && wordCount <= 5 && !hasEmoji) data.category = nextLine;
            else data.category = "None";
          }
        }
      }
