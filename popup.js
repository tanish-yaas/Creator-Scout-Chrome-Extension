...
function parseFollowers(followerString) {
  ...
  let multiplier = 1;
  if (numStr.toLowerCase().endsWith('k')) { multiplier = 1000; numStr = numStr.slice(0, -1); } 
  else if (numStr.toLowerCase().endsWith('m')) { multiplier = 1000000; numStr = numStr.slice(0, -1); }
...
chrome.storage.sync.get(['sheetUrl', 'savedType', 'savedCustomType', 'savedIp', 'ipHistory'], (result) => {
  ...
  if (result.ipHistory) savedIpHistory = result.ipHistory; 
  if (result.savedType) {
    typeSelect.value = result.savedType;
    if (result.savedType === 'Others') {
      customTypeInput.classList.remove('hidden');
      if (result.savedCustomType) customTypeInput.value = result.savedCustomType;
    }
  }
});
...
saveUrlBtn.addEventListener('click', () => {
  ...
  if (url) chrome.storage.sync.set({ sheetUrl: url }, () => showStatus("Database linked!", "success-text"));
});

typeSelect.addEventListener('change', () => {
  chrome.storage.sync.set({ savedType: typeSelect.value });
  if (typeSelect.value === 'Others') customTypeInput.classList.remove('hidden');
  else customTypeInput.classList.add('hidden');
});

customTypeInput.addEventListener('input', () => chrome.storage.sync.set({ savedCustomType: customTypeInput.value }));
...
function renderIpDropdown() {
  ...
  if (filtered.length === 0) { ipDropdown.classList.add('hidden'); return; }
  filtered.forEach(ip => {
    const li = document.createElement('li');
    li.innerText = ip;
    li.addEventListener('mousedown', (e) => { e.preventDefault(); ipInput.value = ip; saveIpToHistory(ip); ipDropdown.classList.add('hidden'); });
...
ipInput.addEventListener('input', () => { chrome.storage.sync.set({ savedIp: ipInput.value }); renderIpDropdown(); });
ipInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); saveIpToHistory(ipInput.value.trim()); ipDropdown.classList.add('hidden'); ipInput.blur(); }});
ipInput.addEventListener('blur', () => { saveIpToHistory(ipInput.value.trim()); ipDropdown.classList.add('hidden'); });

fetchBtn.addEventListener('click', async () => {
  showStatus("Scanning...", "");
  statusDot.className = 'brand-dot dot-yellow';
  let [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (!tab.url.includes("instagram.com")) { showStatus("Error: Open an Instagram profile.", "error-text"); statusDot.className = 'brand-dot'; return; }
  chrome.scripting.executeScript({ target: { tabId: tab.id }, files: ['content.js'] }, () => {
    chrome.tabs.sendMessage(tab.id, { action: "scrape_profile" }, (response) => {
      if (chrome.runtime.lastError || !response || response.status === "error") { showStatus("Scan failed.", "error-text"); statusDot.className = 'brand-dot'; return; }
      ...
      tierElement.innerText = tierInfo.name; tierElement.className = "pill " + tierInfo.class; 
      showStatus("Data fetched!", "success-text"); sendBtn.classList.remove('hidden');
    });
  });
});
...
sendBtn.addEventListener('click', async () => {
  ...
  try {
    const response = await fetch(sheetUrl, { method: "POST", headers: { "Content-Type": "text/plain" }, body: JSON.stringify(currentProfileData) });
    const result = await response.json();
    if (result.status === "Success") { showStatus("Pushed to Sheet!", "success-text"); sendBtn.innerText = "Push to Sheet"; statusDot.className = 'brand-dot dot-green'; }
    else throw new Error();
  } catch (error) { showStatus("Network error.", "error-text"); sendBtn.innerText = "Push to Sheet"; statusDot.className = 'brand-dot'; }
});

function showStatus(text, className) { statusMsg.innerText = text; statusMsg.className = className; }
